...
Page({
  data: {
    ...
    is_checker: false ,
    // 新版头像昵称填写
    avatarUrl: '../../image/tabBar/my@select.png',
    nickName: '',
    canLogin: false,
    show: false,
  },
...
onCloseLogout() {
  ...
  // 检查是否可以登录
  checkCanLogin() {
    const canLogin = this.data.avatarUrl && this.data.nickName && this.data.nickName.trim() !== '';
  ...
  doLogin(e) {
    ...
    wx.setStorage({ key: "hasUserInfo", data: true });
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.data == true) {
          wx.setStorage({ key: "is_checker", data: true });
          Dialog.confirm({
            title: '检票员模式',
            message: '您是检票员，要进入管理页面吗？',
          })
            .then(() => {
              wx.navigateTo({ url: '/pages/checker_page/checker_page' });
            })
            .catch(() => {});
        } else {
          wx.setStorage({ key: "is_checker", data: false });
        }
      }
    });
  },
